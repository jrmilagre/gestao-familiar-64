...
Public Sub testImport()
    ...
    mGitHub.importVbaCode vbaProject
End Sub
...
Public Sub ExportarCodigo() ' Antiga textExport
    ...
    Dim vbaProject As Object
    Set vbaProject = Application.VBE.VBProjects(proj_name)
    mGitHub.exportVbaCode vbaProject
End Sub
...
Public Sub importVbaCode(vbaProject As VBProject, Optional includeClassFiles As Boolean = False)
    ...
    Next
    'Then import them
    Debug.Print "Invoking 'mGitHub.importComponents'with Application.Ontime with delay " & IMPORT_DELAY
    ' to prevent duplicate modules, like MyClass1 etc.
    Application.OnTime Now() + TimeValue(IMPORT_DELAY), "'mGitHub.importComponents'"
