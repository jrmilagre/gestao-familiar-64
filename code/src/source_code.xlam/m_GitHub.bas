...
Public Sub testImport()
    ...
    m_GitHub.importVbaCode vbaProject
End Sub


Public Sub ExportarCodigo() ' Antiga textExport
    Dim proj_name As String
    proj_name = "VbaCode"

    Dim vbaProject As Object
    Set vbaProject = Application.VBE.VBProjects(proj_name)
    m_GitHub.exportVbaCode vbaProject
End Sub
...
Public Sub importVbaCode(vbaProject As VBProject, Optional includeClassFiles As Boolean = False)
    ...
    Next
    'Then import them
    Debug.Print "Invoking 'm_GitHub.importComponents'with Application.Ontime with delay " & IMPORT_DELAY
    ' to prevent duplicate modules, like MyClass1 etc.
    Application.OnTime Now() + TimeValue(IMPORT_DELAY), "'m_GitHub.importComponents'"
...
Private Sub checkHowToImport(file As Object, includeClassFiles As Boolean)
    ...
    If componentName = "m_GitHub" Then
